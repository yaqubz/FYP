...
// To compile: 
// 1. run "cmake ." in nlink_unpack-master directory to generate build files
// 2. in the same directory, run "cmake --build . --config Release" to generate .exe file

// To run:
// In command line, inside the "Build/Release/" directory, run nlink_unpack.exe
